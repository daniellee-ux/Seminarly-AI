...
/// `-prerelease` suffix) used to compare the running build against the latest
/// GitHub release.
///
/// Parsing is deliberately lenient about the things release tags vary on: a
/// leading `v`, missing minor/patch components, surrounding whitespace, and
/// `+build` metadata are all tolerated. Ordering, however, is strict SemVer —
/// numeric (so `0.1.1 < 0.1.10`, never lexical) with full pre-release precedence
/// (`1.0.0-beta < 1.0.0`).
struct SemanticVersion: Equatable, Comparable, CustomStringConvertible {
    ...
    /// Pre-release precedence per SemVer §11.4: compare identifiers left to right;
    /// numeric identifiers compare numerically and always rank below alphanumeric
    /// ones; if all shared identifiers are equal, the shorter set ranks lower.
