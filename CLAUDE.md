...
- **ChatGPT connection component** — `scripts/embed-chatgpt-runtime.sh` bundles the pinned official App Server with SHA-256 verification and license notices. Clean source builds download it once into ignored `build/chatgpt-runtime/`; users never need an external CLI. The build signs it before the app, and packaging verifies both architectures and hardened runtime. Keep the pinned artifacts and protocol smoke tests in sync; see [`docs/chatgpt-plan.md`](docs/chatgpt-plan.md).
