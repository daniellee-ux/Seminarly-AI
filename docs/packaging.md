...
- Automatic checks and downloads remain default-off and opt-in, once a day. An app-owned timer and wake observer continue checking with every window closed. Sparkle probes the signed architecture-specific feed and selects the compatible full or delta archive; background downloads are cached without extracting or starting an installer. A dismissible banner and Settings show when the download is ready. Disabling the preference cancels a pending download.
- Installation requires an explicit action in Sparkle's standard UI; the background cache never arms install-on-quit. During a manual update session, a short-lived loopback server lets Sparkle's XPC downloader reuse only the exact cached enclosure (URL, build and length must match). Sparkle still validates the original archive signature before extraction and handles delta fallback. A changed release or unavailable cache uses the normal download path. No system profile is sent.
